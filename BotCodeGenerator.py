    ...
    def _generate_system_handlers(self) -> str:
        ...
        if "custom_commands" in self.features:
            for cmd in self.features["custom_commands"]:
                desc = self.features["custom_commands"][cmd].get("description", "No description")
                safe_desc = desc.replace('"', '\\"').replace("\n", "\\n")
                command_list.append(f"{cmd} - {safe_desc}")

        help_text = '\\n'.join(["Available commands:"] + command_list) if command_list else "No commands available"
...
"""
    ...
    def _generate_conversation_handlers(self) -> tuple:
        ...
        for cmd, details in self.features["custom_commands"].items():
            ...
            if current_idx < len(session_items) and session_items[current_idx]["type"] == "wait_for_response":
                prompt = session_items[current_idx].get("prompt", "Please respond:").replace('"', '\\"')
                entry_handler += (
            ...
            for idx, state_num in states.items():
                ...
                if next_idx < len(session_items) and session_items[next_idx]["type"] == "wait_for_response":
                    prompt = session_items[next_idx].get("prompt", "Please respond:").replace('"', '\\"')
                    state_handler += (
                        f'    await update.message.reply_text("{prompt}")\n'
            ...
            states_code = [f"{state_num}: [MessageHandler(filters.TEXT & ~filters.COMMAND, {cmd_name}_state_{state_num})]," 
    ...
    def generate_main_keyboard(self) -> str:
        ...
        code.append("async def handle_main_keyboard(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:")
        ...
        for b in buttons:
            ...
            if isinstance(resp, str) and resp.strip().lower() == "/start":
                ...
                code.append(
                    "        '{0}': lambda u,c: context.application._handlers[0][('{1}', None)][0].callback(u,c),"
                    .format(txt, cmd)
                )
    ...
    def _generate_main_function(self, command_handlers: str, callback_handlers: list, text_registrations: list) -> str:
    ...
    def _generate_text_trigger_handlers(self) -> Tuple[str, List[str]]:
        ...
        for trigger, details in self.features["custom_commands"].items():
            ...
            raw_response = details.get("response", "").replace('"', '\\"').replace("\n", "\\n")
        ...
        handlers.append("\n".join(handler_code))
        registrations.append('    application.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, text_trigger_handler))')
    ...
    def _generate_ready_made_handlers(self) -> tuple:
        ...
        # handlers.append(self._generate_template_handler())
        ...
        for cmd, details in self.features.get("custom_commands", {}).items():
            ...
            if not details.get("session") and details.get("response"):
                response_text = details["response"].replace('"', '\\"').replace("\n", "\\n")
                handlers.append(
                    f"async def {cmd_name}_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:\n"
                    f'    """{description}"""\n'
                    "    user_id = update.message.from_user.id\n"
                ...
                for item in details["session"]:
                    if item["type"] == "text":
                        ...
                        if item.get("buttons"):
                            ...
                            button_lines = []
                            for btn in item["buttons"]:
                                btn_text = btn["text"].replace('\\', '\\\\').replace('"', r'\"')
                                btn_url = btn["url"].replace('\\', '\\\\').replace('"', r'\"')
        ...
        return "\n".join(handlers), "\n".join(command_handlers), callback_handlers, text_registrations
    ...
    def generate_code(self) -> str:
        ...
        command_handlers, cmd_reg, cb_reg, text_reg = self._generate_ready_made_handlers()
        code_parts.extend([
            "",
